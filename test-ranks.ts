...
async function main() {
	...
	data.deployNow.slice(0, 10).forEach((s, i) => {
		console.log(
			`${i + 1}. ${s.ticker.padEnd(8)} | Rank: ${String(s.deploymentRank?.toFixed(1)).padEnd(5)} | Base CAGR: ${String(s.baseCagr).padEnd(4)}% | QCS: ${String(s.qcs?.totalScore).padEnd(4)} | Quality: ${String(s.metrics?.roe)} ROE / ${s.metrics?.fcfYield} FCFY`
		);
	});
	console.log('\n--------------------------------------------------------\n');
}
