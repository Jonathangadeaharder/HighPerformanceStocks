const fs = require('fs');
const stocks = fs
	.readdirSync('data/stock-records')
	.filter((f) => f.endsWith('.json'))
	.map((f) => require('../../data/stock-records/' + f));

const subset = stocks.filter((s) =>
	['DEPLOY', 'OVERPRICED', 'WAIT'].includes(s.deployment?.status)
);
...
for (const s of subset) {
	...
	console.log(
		`${s.ticker.padEnd(8)} | ${status} | ${engine} | Score: ${score} | ${multType}: ${mult} | Gr/CAGR: ${growth} | 6m: ${return6m}%`
	);
}
