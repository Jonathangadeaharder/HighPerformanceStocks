<script lang="ts">
	...
	function ink(doc: jsPDF, c: [number, number, number]) {
		doc.setTextColor(...c);
	}
	...
	function drawNotSelectedStock(doc: jsPDF, stock: FindingStock, yPos: number): number {
		...
		doc.setFontSize(8);
		doc.text(
			`${stock.name ?? ''}  ·  ${sc?.engine ?? 'N/A'}  score ${sc?.score ?? 'N/A'}  ·  Base ${stock.baseCagr == null ? 'N/A' : `${stock.baseCagr}%`}  Bear ${stock.bearCagr == null ? 'N/A' : `${stock.bearCagr}%`}`,
			55,
			yPos
		);
		yPos += 5;
	...
	function drawMethodologyPage(doc: jsPDF) {
		...
		doc.text(
			`${deployNow.length} stocks deployed  ·  ${deployNow.length + cheapWait.length + watchlist.length} stocks screened`,
			14,
			27.5
		);
		...
		ink(doc, C.slate);
		doc.text(
			'Every stock passes through three sequential gates before being marked for deployment.',
			14,
			50
		);
		...
		gateBox(
			...
			'(3)',
			'RANK & DEPLOY',
			['Multi-factor score:', 'Valuation + Returns', '+ Momentum + Quality', '+ Conviction (QCS)'],
			C.amber,
			C.amberL
		);
	...
	function drawScreenerEngines(doc: jsPDF, pw: number): void {
		...
		const engines = [
			...
			[
				'fEVG',
				'EV/EBITDA / EPS Growth x Risk',
				'Serial acquirers (amortization distorts earnings).'
			],
		...
		doc.setFontSize(7);
		doc.text(
			'Risk = analyst estimate dispersion multiplier: 1 + 0.6 x (CV - 0.08), where CV = (high - low) / 4 / avg estimate. Defaults to 1.0 when dispersion unavailable.',
			14,
			y
		);
		y += 4;
		doc.text(
			'12% = total return hurdle for income stocks (Yield + Growth). Score 1.0 = exactly 12%; below 1.0 = above hurdle (PASS); above 1.0 = below hurdle (FAIL).',
			14,
			y
		);
	}
	...
	function drawDataSources(doc: jsPDF, pw: number): void {
		...
		doc.setFontSize(8);
		doc.text(
			'Yahoo Finance - live prices, analyst targets (low/mean/high), EPS estimates, revisions, earnings history',
			18,
			y + 12
		);
